...
const RewardsScreen = ({ navigation, route }) => {
	...
	const handleRedeemMedicationCoupon = () => {
		if (medicationCount == MEDICATION_THRESHOLD) {
			...
			Alert.alert(
				'Congratulations!',
				`Your medication reward coupon code is: ${medicationCoupon}`
			);
		} else {
			const remaining = (MEDICATION_THRESHOLD - medicationCount) / 2;
			Alert.alert(
				'Keep Going!',
				`Log ${remaining} more medications to unlock your reward!`
			);
		}
	};
	...
	const handleRedeemConversationCoupon = () => {
		if (conversationCount == CONVERSATION_THRESHOLD) {
			...
			Alert.alert(
				'Congratulations!',
				`Your conversation reward coupon code is: ${conversationCoupon}`
			);
		} else {
			const remaining = CONVERSATION_THRESHOLD - conversationCount;
			Alert.alert(
				'Keep Going!',
				`Participate in ${remaining} more conversations to unlock your reward!`
			);
		}
	};
	...
	return (
		<View style={styles.container}>
			...
			<Button
				title={
					medicationCoupon
						? 'Redeem Medication Coupon'
						: 'Check Medication Progress'
				}
			...
			<Text style={styles.subtitle}>
				Conversations Participated: {conversationCount} /{' '}
				{CONVERSATION_THRESHOLD}
			</Text>
			<Button
				title={
					conversationCoupon
						? 'Redeem Conversation Coupon'
