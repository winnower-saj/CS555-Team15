...
const RewardsScreen = ({ navigation,route }) => {
  ...
  const MEDICATION_THRESHOLD = 10; // Adjust for medication reward threshold
  const CONVERSATION_THRESHOLD = 5; // Adjust for conversation reward threshold
  ...
  const handleTrackMedication = () => {
    if (medicationCount < MEDICATION_THRESHOLD) {
      const updatedCount = medicationCount + 1;
      setMedicationCount(updatedCount);

      if (updatedCount >= MEDICATION_THRESHOLD) {
        setMedicationCoupon(generateCouponCode());
      }
    }
  };

  const handleTrackConversation = () => {
    if (conversationCount < CONVERSATION_THRESHOLD) {
      const updatedCount = conversationCount + 1;
      setConversationCount(updatedCount);

      if (updatedCount >= CONVERSATION_THRESHOLD) {
  ...
  return (
    <View style={styles.container}>
      ...
      <Button title="Log Medication" onPress={handleTrackMedication} />
      <Button
      ...
      <Button title="Log Conversation" onPress={handleTrackConversation} />
      <Button
